				...
				$.post('/fileManager/uptatePwd',{'_token':token,'data':obj},function(data){
				 	var data = JSON.parse(data);
						if(data == false) {
							...
							window.location.href="/platform/platform-email";
					}
				});
			}else{
				$.post('/todayWork/uptatePwd',{'_token':token,'data':obj},function(data){
				 	var data = JSON.parse(data);
						if(data == false) {
							$("input[name='pwd']").addClass('warning');
							$("input[name='pwd']").val('');
	...
	$("#bind-email-btn").click(function(event){
		...
		if($("#email").next(".isRight").hasClass("right")){
			...
			$.post('/emailcode',{'_token':token,'data':info.code},function(data){
				if (data == 200) {
					$.post('/platformEmail',{'_token':token,'data':info},function(data){
					 	email = $("#email").val();
						$(".addInfo .myForm").hide();
						$(".bind-email-box").show();
...
$(function(){
	...
	//加载"管理员档案"引导
		...
		$("body").on("click","#left .guide-active .part button",function(){
			$("#nav1>li:first-child>a").css({color: "#333"}).attr("href","/adminArchives/manager-archives/manager-list");
		});
	}
	//“点击此处”跳转
